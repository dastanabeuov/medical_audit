...
class DoctorDataExtractorService
  ...
  # Паттерны для поиска ФИО врача (поддержка казахских букв)
  DOCTOR_PATTERNS = [
    # Стандартные паттерны: "Врач:", "Доктор:", "Лечащий врач:"
    /(?:Врач|Доктор|Лечащий\s+врач|Специалист)[\s:]+((#{CYRILLIC_PATTERN}+)\s+(#{CYRILLIC_PATTERN}+)(?:\s+(#{CYRILLIC_PATTERN}+))?)/io,
    /(?:Провел\s+консультацию|Принимающий\s+врач)[\s:]+((#{CYRILLIC_PATTERN}+)\s+(#{CYRILLIC_PATTERN}+)(?:\s+(#{CYRILLIC_PATTERN}+))?)/io,

    # Паттерны со специальностями: "Педиатр:", "Уролог:", "Терапевт:" и т.д.
    /(?:Педиатр|Уролог|Терапевт|Хирург|Кардиолог|Невролог|Офтальмолог|Отоларинголог|Гинеколог|Дерматолог|Эндокринолог|Гастроэнтеролог|Пульмонолог|Нефролог|Ревматолог|Онколог|Психиатр|Психолог|Стоматолог|Ортопед|Травматолог)[\s:]+((#{CYRILLIC_PATTERN}+)\s+(#{CYRILLIC_PATTERN}+)(?:\s+(#{CYRILLIC_PATTERN}+))?)/io
  ].freeze
  ...
  class << self
    ...
    def extract_doctor_info(content)
      ...
      DOCTOR_PATTERNS.each do |pattern|
        ...
        # Ищем слово перед двоеточием в найденном совпадении (поддержка казахских букв)
        full_match = match[0]
        specialization_match = full_match.match(/^(#{CYRILLIC_PATTERN}+)[\s:]+/o)
